...
async def select_vehicles(
    ...
    for attempt in range(3):
        ...
        valid_ids = [vehicle_id for vehicle_id in ids if vehicle_id in checkbox_map and not state.is_locked(vehicle_id)]
        if valid_ids:
            break
        if attempt < 2:
            try:
    ...
    vehicles = [
        (vehicle_id, parse_distance_value(distance_map.get(vehicle_id, float("inf")))) for vehicle_id in valid_ids
    ]
...
async def find_vehicle_ids(name: str, profile=None, state=None, exact=False, quiet=False):
    ...
    normalized_names = {variant for item in names if item for variant in vehicle_name_variants(item)}
    for vehicle_type, vehicle_ids in vehicle_data.items():
        ...
        decorated_match = (not exact) and is_decorated_vehicle_name(normalized_names, vehicle_type)
