...
async def read_water_status(page):
    ...
    need = int(need_attribute) if need_attribute and need_attribute.isdigit() else 0
    match = re.search(r"Selected:\s*([\d,]+)\s*gal", await bar.inner_text())
    selected = int(match.group(1).replace(",", "")) if match else 0
