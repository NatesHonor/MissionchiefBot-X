...
async def gather_vehicle_data(contexts, num_threads):
    ...
    try:
        ...
        await page.goto("https://www.missionchief.com/leitstellenansicht", wait_until="domcontentloaded")
        ...
        vehicle_data = await split_vehicle_ids_among_threads(vehicle_ids, contexts, threads)
...
async def gather_vehicle_info(vehicle_ids, context, thread_id):
    vehicle_data = {}
    ...
    for index, vehicle_id in enumerate(vehicle_ids):
        try:
            ...
            await page.goto(f"https://www.missionchief.com/vehicles/{vehicle_id}", wait_until="domcontentloaded")
...
async def split_vehicle_ids_among_threads(vehicle_ids, contexts, num_threads):
    threads = min(num_threads, len(contexts)) if num_threads else len(contexts)
    ...
    tasks = [
        gather_vehicle_info(partitions[i], contexts[i], i + 1)
